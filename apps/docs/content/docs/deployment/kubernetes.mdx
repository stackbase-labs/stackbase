...
Stackbase ships a complete set of Kubernetes manifests in the `k8s/` directory, plus a `deploy.sh` script that builds and pushes your Docker images and applies the manifests, and a `verify.sh` script that checks the rollout afterward.
...
| File                     | Purpose                                                           |
| ------------------------ | ----------------------------------------------------------------- |
| `k8s/namespace.yml`      | Dedicated `stackbase` namespace                                   |
...
Internet → nginx Ingress (:80/:443)
         ├─ /api/v1/*  → rewrite /api/v1/* → /api/* → stackbase-api-service (:4000) → Express API
         └─ /*         → stackbase-web-service (:3000) → Next.js pages

Next.js server-side fetches (SSR) reach the API in-cluster via
API_INTERNAL_URL → stackbase-api-service (:4000), bypassing the ingress.
```
...
<Callout type="info">
  ...
  issue on [GitHub](https://github.com/stackbase-labs/stackbase/issues).
</Callout>
