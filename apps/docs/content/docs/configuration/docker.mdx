...
services:
  postgres:
    ...
    container_name: stackbase-postgres-dev
    environment:
      POSTGRES_DB: stackbase-app
      POSTGRES_USER: postgres
      POSTGRES_PASSWORD: password
    ports:
      - 5432:5432
    volumes:
      - stackbase-app_postgres_data_dev:/var/lib/postgresql/data
    healthcheck:
...
<Callout type="warning">
  The development docker-compose uses hardcoded credentials for convenience.
...
docker-compose exec postgres psql -U postgres -d stackbase-app
...
Prometheus and Grafana can be started with the included observability compose file:
...
See the [Observability](/docs/configuration/observability) guide for the full setup, dashboard, and troubleshooting notes.
...
# Connect to database
docker-compose -f docker-compose.prod.yml exec postgres psql -U postgres -d stackbase
