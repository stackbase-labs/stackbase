---
title: Environment Variables
description: Secure configuration management across development, staging, and production environments using .env files and proper secret handling
---

# Environment Variables

Environment variables securely manage configuration across different environments (development, production) without committing sensitive data to version control.

## Overview

Each application and package uses `.env` files for environment-specific configuration:

- **`.env.local`** - Local development variables (gitignored)
- **`.env.example`** - Template showing required variables
- **`.env.production`** - Production environment variables

These files are **not** committed to version control and should be created from the provided `.env.example` files.

## Setup

1. Copy the relevant `.env.example` file to `.env`, `.env.local`, or `.env.production` in each app/package.

```bash tab="macOS/Linux"
find . -name ".env.example" -execdir sh -c 'cp .env.example .env' \;
```

```powershell tab="Windows"
Get-ChildItem -Recurse -Filter .env.example | ForEach-Object {
	Copy-Item $_.FullName ($_.DirectoryName + "\.env") -Force
}
```

2. Fill in the required values in each `.env` file as described below.

## Quick Start

To run locally with basic functionality, configure these **required** variables:

1.  **Database**

```
DATABASE_URL=postgres://postgres:password@localhost:5432/app
```

<Callout type="info">
  For local development, we recommend [Supabase](https://supabase.com/) for a
  managed PostgreSQL database. Set up a free project and get the connection
  string.
</Callout>

2.  **Authentication Secret**

Add to both `apps/api/.env.local` and `apps/web/.env.local` (must be identical):

- `BETTER_AUTH_SECRET=your-generated-secret`

Generate a secure secret for Better Auth:

```bash
# Option 1: Using Node.js
node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"

# Option 2: Using OpenSSL
openssl rand -hex 32

# Option 3: Online generator
# Visit: https://generate-secret.vercel.app/32
```

<Callout type="warning">
  The `BETTER_AUTH_SECRET` must be **identical** in both `apps/api/.env.local`
  and `apps/web/.env.local`. Copy and paste the same generated value in both
  files.
</Callout>

3.  **OAuth Credentials**

Add to `apps/web/.env.local` and `apps/api/.env`:

- `GOOGLE_CLIENT_ID=your-google-client-id`
- `GOOGLE_CLIENT_SECRET=your-google-client-secret`
- `NEXT_PUBLIC_BASE_URL=http://localhost:3000`

<Callout type="info">
  Set up OAuth credentials in the [Google Cloud
  Console](https://console.cloud.google.com/apis/credentials). Use
  `http://localhost:3000/api/auth/callback/google` as the redirect URI.
</Callout>

4.  **Email Service (Resend)**

Add to `apps/web/.env.local`, `apps/api/.env.local` and `packages/email/.env.local`:

- `RESEND_TOKEN=your-resend-token`
- `RESEND_EMAIL_FROM=your@email.com`

<Callout type="info">
  Sign up at [Resend](https://resend.com/) to get an `API token` and set up a
  verified sender email.
</Callout>

5.  **Rate Limiting**

Add to `apps/api/.env.local` and `packages/rate-limit/.env.local`:

    - `UPSTASH_REDIS_REST_URL=your-upstash-url`
    - `UPSTASH_REDIS_REST_TOKEN=your-upstash-token`

<Callout type="info">
  Use [Upstash](https://upstash.com/) for a managed Redis instance. Set up a
  free database and get the `REST_URL` and `TOKEN`.
</Callout>

## Optional Variables

These are not required to run locally - sensible defaults apply.

**Logging** (`apps/api/.env.local`)

- `LOG_LEVEL` — minimum log level to emit. Defaults to `info` in production and `debug` in development. Accepts `trace`, `debug`, `info`, `warn`, `error`, `fatal`, or `silent`.

```bash
# Quiet a noisy environment, or raise verbosity when debugging
LOG_LEVEL=warn
```

See the [Logger Package](../packages/logger) for output formats and usage.

6. **File Storage (S3 / Cloudflare R2 / MinIO)**

- `S3_BUCKET` — bucket name for asset and upload storage.
- `S3_REGION` — AWS region or `auto` for Cloudflare R2. Defaults to `us-east-1`.
- `S3_ACCESS_KEY_ID` — access key identifier for S3 / R2 / MinIO.
- `S3_SECRET_ACCESS_KEY` — secret access key.
- `S3_ENDPOINT` — optional custom S3-compatible API endpoint (e.g. `http://localhost:9000` or `https://<accountid>.r2.cloudflarestorage.com`).
- `S3_PUBLIC_URL` — optional public domain or CDN URL for reading uploaded assets.
- `S3_FORCE_PATH_STYLE` — optional boolean (set to `true` for MinIO).

See the [Storage Package](../packages/storage) for complete setup guides and CORS rules.

## Environment Variable Files

Stackbase uses the following `.env` files:

| File                             | Description                                  |
| -------------------------------- | -------------------------------------------- |
| `apps/web/.env.local`            | Web Application Variables                    |
| `apps/api/.env.local`            | API Server Variables                         |
| `packages/db/.env.local`         | Database connection and Prisma config        |
| `packages/auth/.env.local`       | Better Auth secret & OAuth credentials       |
| `packages/email/.env.local`      | React Email templates and Resend integration |
| `packages/rate-limit/.env.local` | Upstash Redis config                         |
| `packages/storage/.env.local`    | S3, Cloudflare R2, or MinIO storage config   |

## Adding New Variables

To add new environment variables:

1. Define the variable in the appropriate `.env` file.
2. Add validation to the server or client object in the package's `keys.ts` file.

Example `packages/package-name/keys.ts`:

```ts
import { createEnv } from "@t3-oss/env-nextjs";
import { z } from "zod";
export const keys = createEnv({
  server: {
    NEW_SECRET: z.string().min(1),
  },
  client: {
    NEXT_PUBLIC_NEW_VALUE: z.string().optional(),
  },
  runtimeEnv: {
    NEW_SECRET: process.env.NEW_SECRET,
    NEXT_PUBLIC_NEW_VALUE: process.env.NEXT_PUBLIC_NEW_VALUE,
  },
});
```

## Deployment

When deploying to production:

1. Ensure all required environment variables are set in your hosting platform (e.g., Vercel, Heroku, etc.). Refer to their documentation for setting environment variables.

2. Update URL variables (`NEXT_PUBLIC_APP_URL`, etc.) to production values as needed.

<Callout type="info">
  Variables prefixed with `VERCEL_` are automatically available in Vercel
  deployments, such as `VERCEL_PRODUCTION_URL`.
</Callout>
