...
10. Deletes internal-only content (`packages/stackbase/`, `assets/`, `apps/docs/`, `SCREENSHOTS.md`, etc.).
...
<Callout type="warn">
  ...
  `init` command. It requires a `.stackbase.json` manifest. Legacy
  `.build-elevate.json` manifests are still supported.
</Callout>
...
- `packages/stackbase/`, `assets/` - internal CLI content, deleted during `init`
