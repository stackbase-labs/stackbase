...
2. Clones `templates/base` from the repository via `degit` (no `.git` history).
...
5. Replaces Stackbase project identifiers with your project name across all relevant files.
...
10. Runs the internal-content cleanup guard before writing the project manifest.
11. Writes a `.stackbase.json` manifest recording the commit SHA, template, project name, and a SHA-256 hash of every tracked file in its final post-mutation state.
...
- Internal repository files outside `templates/base/`
